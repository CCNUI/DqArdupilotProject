1、我现在有6轴（非对称，非同一高度布局）的倾转旋翼飞机。请告诉我要如何调整Ardupilot V4.6.2以实现适配。请详细说明步骤和可能用到的设备、代码，注意我要能直接放上去运行的代码而不是简略或是框架。我的飞控型号是H743。
2、但是它这个是非对称、非同一水平线的，要如何计算呢？而且它还要倾转，控制倾转舵机前后和倾转过程不会出错？舵机舵面控制不会出错？另外在上升时为了减少阻力所有舵面都要向下打，平飞却要能正常控制
3、另外的问题（先回答前面的问题再回答这个）：某些舵机的设计倾转不会到90度（平飞时是平的，但是vtol转垂起不会到90度。而且只是部分电机。有的是会到90度的），如何调整Ardupilot V4.6.2以适配？
我希望你能够尽可能地利用原有的VTOL文件和代码，尽可能使用Mission Planner原有功能尽量少地自己写以提高可靠性。
另外给你几个文件：
/*
